...
test("커밋 출처: source=commit + commit 메타로 오고, 같은 sha 는 한 번만 분석된다", async () => {
  const alice = client("alice", "api", [payIdx]);
  const bob = client("bob", "web", [clientIdx]);
  await Promise.all([alice.ready, bob.ready]);
  await sleep(150);
  ...
  const commits = bob.inbox.filter((m) => m.type === "impact" && m.source === "commit");
  assert.equal(commits.length, 1, "commit impact 가 정확히 1건이어야 함");
  ...
  // 같은 sha·파일 재전송 → 중복 분석 안 함 (여러 클라가 같은 커밋을 보고)
  alice.saveCommit("src/pay.ts", SIG_DIFF, commit);
  await sleep(400);
  assert.equal(
    bob.inbox.filter((m) => m.type === "impact" && m.source === "commit").length,
    1,
...
test("푸시 출처: source=push 로 구분돼 오고, ref 메타가 전달된다", async () => {
  ...
  const pushes = bob.inbox.filter((m) => m.type === "impact" && m.source === "push");
