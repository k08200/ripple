...
**인증(공유 백엔드면 권장)**: 두뇌를 `RIPPLE_SECRET=어떤토큰 npm run brain` 으로 띄우고,
각 팀원은 VS Code 설정 `ripple.secret` 에 같은 토큰을 넣는다. 토큰이 틀리면 연결이 거부된다.
(미설정 시 인증 없이 개방 — localhost 단독 테스트용)
...
- **두뇌는 in-memory + 전체 broadcast** — 변경 요약이 접속한 모두에게 흐른다. **한 팀(신뢰 그룹) 규모용**. 인증은 `RIPPLE_SECRET` 으로(위 참고), 전사/외부 노출 전엔 라우팅 격리도 필요.
- **비밀 파일 보호**: `.env`/`.pem`/`.key`/이름에 secret·credential 포함 파일은 분석·외부 LLM 전송에서 자동 제외.
- **LLM 모드 주의**: `RIPPLE_PROVIDER=hybrid|openrouter|claude` 면 변경 diff 가 외부 API 로 전송된다(기본 `graph` 는 전송 0, 로컬 결정론). 사내 코드면 기본 graph 권장.
